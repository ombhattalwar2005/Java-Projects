...
public class Sorters2120 {
    ...
    public static <T extends Comparable<T> > void selectionSort(List<T> theList)
    {
        ...
        for(startIndex = 0; startIndex < theList.size()-1; startIndex++)
        {
            ...
            else if(minElement.compareTo(theList.get(comparedIndex)) == 1)//greater th
            {
    ...
    private static <T extends Comparable<T> > void recursiveMergeSortHelper(List<T> theList, int first, int last)
    {
        ...
        {
            int middle = (last-1)/2;
            if(theList.get(first).compareTo(theList.get(last-1)) == -1)//if its in order or the 
            {
                if(first < middle)
                {
            ...
            else if(theList.get(first).compareTo(theList.get(last-1)) == 1)//if its out of orde
            {
